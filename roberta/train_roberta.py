...
# !!运行前需要修改   模型保存位置以及输出位置
parser.add_argument('--output_dir', default='outputs/roberta/politifact')
...
# zhuzhi 2024/6/20
# 这里手动下载好了模型/data/zz/FakingFakeNews/model/roberta-large
...
# zhuzhi 2024/6/24 修改读取文件
# !!运行前需要修改  init loader  
train_set = PropaFakeDataset(os.path.join(args.data_dir,'politifact_train.jsonl'))
train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, collate_fn=train_set.collate_fn)
dev_set = PropaFakeDataset(os.path.join(args.data_dir,'politifact_valid.jsonl'))
...
for epoch in range(args.max_epoch):
    ...
    with torch.no_grad():
        ...
        dev_true_accuracy = torch.sum(dev_labels_bool[dev_labels_bool == 1] == dev_outputs_bool[dev_labels_bool == 1]) / torch.sum(dev_labels_bool == 1)
        dev_false_accuracy = torch.sum(dev_labels_bool[dev_labels_bool == 0] == dev_outputs_bool[dev_labels_bool == 0]) / torch.sum(dev_labels_bool == 0)
...
# 这里保存准确率最好的模型
        ...
        print(f"验证集结果： Epoch {epoch} 真新闻准确率：{dev_true_accuracy * 100:.2f}，假新闻准确率：{dev_false_accuracy * 100:.2f}， 分类准确率: {dev_accuracy * 100:.2f}. Best dev accuracy: {best_dev_accuracy*100:.2f}.")            
